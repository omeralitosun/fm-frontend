  ...
  export default {
    ...
    data() {
      return {
        ...
        fields: null,
        putHarvestUrl: process.env.VUE_APP_API_BASE_URL + "/api/v1/harvest/",
        getHarvestUrl: process.env.VUE_APP_API_BASE_URL + "/api/v1/harvest/" + this.id,
    ...
    methods: {
      submit: function () {
        ...
        fetch(this.putHarvestUrl + this.harvestId, requestOptions)
          .then((response) => {
            if (response.ok) {
      ...
      fetch(this.getHarvestUrl, requestOptions)
        .then((response) => response.json())
        .then((data) => {
          _this.harvestId = data.id;
          _this.harvest.name = data.name
          _this.harvest.amount = data.amount;
          _this.harvest.unitPrice = data.unitPrice;
          _this.harvest.comment = data.comment;
          _this.harvest.date = data.date;
          _this.harvest.fieldId = data.field? data.field.id:"";
        });
      }
    },
    created() {
